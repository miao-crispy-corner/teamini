...
import (
	...
	"github.com/miao-crispy-corner/teamini/pkg/db"
)

const (
	// recommendedHomeDir 定义放置 teamini 服务配置的默认目录.
	recommendedHomeDir = ".teamini"

	// defaultConfigName 指定了 teamini 服务的默认配置文件名.
	defaultConfigName = "teamini.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 teamini，如果是 teamini，将自动转变为大写。
	viper.SetEnvPrefix("teamini")
...
// initStore 读取 db 配置，创建 gorm.DB 实例，并初始化 teamini store 层.
